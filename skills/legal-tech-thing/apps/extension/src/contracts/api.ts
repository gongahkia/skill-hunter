...
const SOURCE_TYPE = "EXTENSION_DOM";
...
export interface SubmitExtensionContractInput {
  title: string;
  content: string;
}

export interface SubmitExtensionContractResult {
  contractId: string;
...
function toSafeFileName(title: string) {
  ...
  return normalized ? `${normalized}.txt` : "extension-capture.txt";
}
...
export async function submitExtractedContractSource(
  input: SubmitExtensionContractInput,
  accessToken: string
): Promise<SubmitExtensionContractResult> {
